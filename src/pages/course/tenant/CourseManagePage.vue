<template>
  <div class="course-manage">
      ...
      <div class="drawer-body">
        <t-form :data="form" label-width="120px" label-align="right">
          ...
          <div v-if="form.mode === 'recorded'" class="section-card">
            ...
            <!-- V2·0902 老板需求：同步开关移至营期排课页（营期级）；答题触发时机随排课层课时带出，课程设置不再配置 -->
    ...
    <!-- V2·0902 用户裁决：课程设置·答题题库选择弹窗 + 题库详情 -->
    <QuizPickerDialog ref="quizPickerRef" @confirm="onQuizBankPicked" />
    <t-dialog v-model:visible="quizBankDetailVisible" :header="quizBankDetailTitle" width="720px" :footer="false">
      <div v-if="quizBankQuestions.length === 0" style="color:#98A2B3;font-size:13px;padding:12px 0">该题库暂无题目</div>
      <t-table v-else :data="quizBankQuestions" row-key="id" bordered size="small" max-height="360"
        :columns="[
          { colKey: 'serial-number', width: 54, title: '序号' },
          { colKey: 'content', title: '题干', minWidth: 220 },
          { colKey: 'qtype', title: '题型', width: 90 },
          { colKey: 'score', title: '分值', width: 70 },
        ]">
        <template #qtype="{ row }">{{ quizTypeLabel(row.question_type) }}</template>
      </t-table>
    </t-dialog>

    <!-- V2·0829 用户裁决：课时管理/题库管理抽屉入口已随操作列按钮删除（相关操作统一在编辑模块内完成） -->

    <!-- V2·0902 用户裁决（0904 终版）：「查看关联」两层结构——独立录播课程（课程信息+学员进度）/ 营期课程（营期列表→下钻学员表含报名记录） -->
    <t-dialog v-model:visible="campRefVisible" :header="`查看关联 · ${campRefCourse?.title ?? ''}`" width="760px" :footer="false">
      <t-tabs v-model="campRefTab">
        ...
        <t-tab-panel value="camps" label="营期课程">
          <!-- 层1：营期列表 -->
          <template v-if="!campRefDrillCamp">
            ...
            <t-table v-else :data="refCamps" row-key="id" bordered size="small" max-height="380"
              ...
              <template #op="{ row }"><t-button variant="text" size="small" theme="primary" @click="campRefDrillCamp = row">查看学员</t-button></template>
            </t-table>
          </template>
          <!-- 层2：下钻——该营期学员进度表（含报名记录） -->
          <template v-else>
            <div style="margin-bottom:10px">
              <t-button variant="text" size="small" theme="primary" @click="campRefDrillCamp = null"><t-icon name="chevron-left" /> 返回营期列表</t-button>
              <span style="margin-left:8px;font-size:13px;color:#475467">{{ campRefDrillCamp.title }} · 学员进度</span>
            </div>
            <div v-if="drillStudents.length === 0" style="color:#98A2B3;font-size:13px;padding:12px 0">该营期暂无报名学员</div>
            <t-table v-else :data="drillStudents" row-key="key" bordered size="small" max-height="380"
              :columns="[
                { colKey: 'no', title: '学员编号', width: 170 },
                { colKey: 'name', title: '学员', width: 80 },
...
// 层2 下钻：所选营期的学员进度（含报名记录）
const campRefDrillCamp = ref<any>(null);
const drillStudents = computed(() => {
  if (!campRefDrillCamp.value) return [];
  const cid = campRefDrillCamp.value.id;
  return campStore.enrollments
    .filter((e: any) => e.camp_id === cid)
    .map((e: any, i: number) => {
      const lr = campStore.learningRecords.find((r: any) => r.student_id === e.student_id && r.camp_id === cid);
      const rate = lr ? Math.round(lr.completion_rate * 100) + '%' : '-';
      return {
        key: (e as any).id || i, no: (e as any).student_no || e.student_id, name: e.student_name,
...
function openCampRef(row: any) { campRefCourse.value = row; campRefTab.value = 'standalone'; campRefDrillCamp.value = null; campRefVisible.value = true; }
