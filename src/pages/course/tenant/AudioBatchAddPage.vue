<template>
  <div class="batch-add-page">
    <div class="page-header"><div class="page-title"><h2>批量创建音频</h2><span class="page-sub">批量上传音频并统一配置发布信息</span></div></div>
    <div class="stepper">
      <div class="step" :class="{ active: step === 1, done: step > 1 }"><span class="step-num">{{ step > 1 ? '✓' : '1' }}</span><span class="step-label">上传音频</span></div>
      <div class="step-line" :class="{ done: step > 1 }"></div>
      <div class="step" :class="{ active: step === 2, done: step > 2 }"><span class="step-num">{{ step > 2 ? '✓' : '2' }}</span><span class="step-label">填写课节名称</span></div>
      <div class="step-line" :class="{ done: step > 2 }"></div>
      <div class="step" :class="{ active: step === 3 }"><span class="step-num">3</span><span class="step-label">批量设置</span></div>
    </div>
    <t-card v-if="step === 1" :bordered="false" class="section">
      <div v-if="selectedAudios.length === 0" class="empty-step"><t-icon name="sound" class="empty-icon" /><div class="empty-title">还没有选择音频</div><div class="empty-sub">点击下方按钮上传或从素材库选择</div></div>
      <div v-else class="selected-list">
        <div class="list-header"><span>已选择 {{ selectedAudios.length }} 个音频</span><t-button variant="text" theme="danger" size="small" @click="selectedAudios = []">清空</t-button></div>
        <div v-for="item in selectedAudios" :key="item.id" class="list-item">
          <div class="item-thumb" :style="{ background: item.color }"><t-icon name="sound" /></div>
          <div class="item-info"><div class="item-name">{{ item.name }}</div><div class="item-meta">{{ item.duration }} · {{ item.size }}</div></div>
          <t-button variant="text" theme="danger" size="small" @click="removeSelected(item)">移除</t-button>
        </div>
      </div>
      <div class="upload-bar"><t-button theme="primary" variant="outline" @click="showPicker = true"><template #icon><t-icon name="upload" /></template> 上传音频</t-button><span class="upload-tip">单次最多上传 50 个音频</span></div>
    </t-card>
    <t-card v-else-if="step === 2" :bordered="false" class="section">
      <div class="section-hint"><t-icon name="info-circle" /><span>已选择 {{ selectedAudios.length }} 个音频，请为每个音频填写课节名称（默认使用音频文件名）</span></div>
      <div class="chapter-list">
        <div v-for="(item, idx) in selectedAudios" :key="item.id" class="chapter-row">
          <span class="chapter-idx">{{ idx + 1 }}</span>
          <div class="chapter-thumb" :style="{ background: item.color }"><t-icon name="sound" /></div>
          <div class="chapter-source"><div class="chapter-source-name">{{ item.name }}</div><div class="chapter-source-meta">{{ item.duration }}</div></div>
    ...
    <div v-else class="form-main">
      <t-card :bordered="false" class="section">
        <div class="section-head"><span class="section-badge"><t-icon name="check" /></span><div class="section-title-wrap"><h3 class="section-title">批量设置</h3><span class="section-sub">以下配置将应用于全部 {{ selectedAudios.length }} 个音频</span></div></div>
        <div class="field"><div class="field-label">详情设置</div><div class="radios-inline"><t-radio :checked="batchForm.detail_mode === 'full'" @change="batchForm.detail_mode = 'full'">购买前查看完整音频详情</t-radio><t-radio :checked="batchForm.detail_mode === 'brief'" @change="batchForm.detail_mode = 'brief'">购买前仅查看课节介绍</t-radio></div></div>
        <div class="field"><div class="field-label">音频详情（统一简介）</div><RichEditor v-model="batchForm.description" placeholder="为全部音频设置统一的简介描述" :max-length="50000" /></div>
      </t-card>
      <t-card :bordered="false" class="section">
        <div class="section-head"><span class="section-badge"><t-icon name="cart" /></span><div class="section-title-wrap"><h3 class="section-title">商品信息</h3><span class="section-sub">统一售卖方式与价格</span></div></div>
        <div class="field"><div class="field-label">售卖方式</div><t-radio-group v-model="batchForm.sale_type"><t-radio value="free">免费</t-radio><t-radio value="paid">付费</t-radio><t-radio value="member">会员专享</t-radio></t-radio-group></div>
        <div v-if="batchForm.sale_type === 'paid'" class="paid-panel">
          <div class="field"><div class="field-label"><span class="req">*</span>统一售价（元）</div><t-input v-model="batchForm.price" placeholder="请输入价格，最低为1" style="width: 280px" /></div>
          <div class="field"><div class="field-label">划线价格（元）</div><t-input v-model="batchForm.original_price" placeholder="选填" style="width: 280px" /></div>
          <div class="field"><div class="field-label">有效期</div><div class="validity-options"><t-radio :checked="batchForm.paid_validity === 'long'" @change="batchForm.paid_validity = 'long'">长期有效</t-radio><t-radio :checked="batchForm.paid_validity === 'custom'" @change="batchForm.paid_validity = 'custom'">自定义时长</t-radio><t-radio :checked="batchForm.paid_validity === 'fixed'" @change="batchForm.paid_validity = 'fixed'">固定时长（自购买后X天内有效）</t-radio></div>
          ...
          <div v-if="batchForm.paid_validity === 'fixed'" class="fixed-days-row">自购买后 <t-input-number v-model="batchForm.paid_fixed_days" :min="1" :max="3650" style="width: 120px" /> 天内有效</div>
          </div>
        </div>
        <div v-if="batchForm.sale_type === 'free'" class="field"><div class="field-label">有效期</div><span class="validity-static">长期有效</span></div>
        <div v-if="batchForm.sale_type === 'member'" class="member-panel">
          <div class="field"><div class="field-label"><span class="req">*</span>会员可享等级</div><t-radio-group v-model="batchForm.member_scope"><t-radio value="all">所有等级会员可享</t-radio><t-radio value="specific">指定会员等级可享</t-radio></t-radio-group></div>
          <div v-if="batchForm.member_scope === 'specific'" class="field"><div class="field-label">选择会员等级</div><t-select v-model="batchForm.member_levels" multiple placeholder="请选择会员等级" style="width: 100%; max-width: 480px"><t-option label="黄金会员" value="gold" /><t-option label="钻石会员" value="diamond" /><t-option label="星耀会员" value="star" /><t-option label="至尊会员" value="supreme" /></t-select></div>
        </div>
      </t-card>
      <t-card :bordered="false" class="section">
        <div class="section-head"><span class="section-badge"><t-icon name="setting" /></span><div class="section-title-wrap"><h3 class="section-title">音频设置</h3><span class="section-sub">播放规则与内容保护</span></div></div>
        <div class="field"><div class="field-label">播放设置</div><div class="switch-row"><t-switch v-model="batchForm.forbid_seek" /><span class="switch-text">音频未听完，禁止快进</span></div><div class="switch-row"><t-switch v-model="batchForm.forbid_speed" /><span class="switch-text">音频未听完，禁止倍速播放</span></div></div>
        <div class="field"><div class="field-label">内容保护</div><div class="switch-row"><t-switch v-model="batchForm.watermark_horse" /><span class="switch-text">开启防录屏跑马灯</span></div><div class="switch-row"><t-switch v-model="batchForm.watermark_text" /><span class="switch-text">开启水印</span></div></div>
        <div class="field"><div class="field-label">允许试听</div><div class="switch-row"><t-switch v-model="batchForm.allow_preview" /><span class="switch-text">开启后学员可免费试听前 1 分钟</span></div></div>
      </t-card>
      <t-card :bordered="false" class="section">
        <div class="section-head"><span class="section-badge"><t-icon name="upload" /></span><div class="section-title-wrap"><h3 class="section-title">上架设置</h3><span class="section-sub">统一上架策略</span></div></div>
        <div class="field"><div class="field-label"><span class="req">*</span>是否上架</div><t-radio-group v-model="batchForm.on_shelf"><t-radio value="immediate">立即上架</t-radio><t-radio value="scheduled">定时上架</t-radio><t-radio value="takedown">下架</t-radio></t-radio-group></div>
        <div v-if="batchForm.on_shelf === 'scheduled'" class="field"><div class="field-label">定时上架时间</div><t-date-picker v-model="batchForm.scheduled_time" enable-time-picker placeholder="选择日期时间" /></div>
        <div v-if="batchForm.on_shelf === 'immediate' || batchForm.on_shelf === 'scheduled'" class="field"><div class="field-label">定时下架时间</div><t-date-picker v-model="batchForm.takedown_time" enable-time-picker placeholder="选择日期时间" /></div>
        <div class="shelf-tip">音频上架后，可在店铺主页显示；下架时隐藏，下架后学员无法学习该内容。</div>
      </t-card>
    </div>
    <div class="footer-actions"><t-button theme="default" @click="goBack">返回</t-button><t-button v-if="step > 1" theme="default" variant="outline" @click="step--">上一步</t-button><t-button v-if="step === 1" theme="primary" :disabled="selectedAudios.length === 0" @click="step = 2">下一步</t-button><t-button v-if="step === 2" theme="primary" @click="step = 3">下一步</t-button><t-button v-if="step === 3" theme="primary" :loading="submitting" @click="doSave">保存</t-button></div>
    <t-dialog v-model:visible="showPicker" header="上传音频" width="720px" :footer="false">
      ...
      <div class="picker-footer"><span class="picker-count" v-if="pickerSelectedKeys.length > 0">已选 {{ pickerSelectedKeys.length }} 个</span><t-button theme="default" @click="showPicker = false">取消</t-button><t-button theme="primary" @click="confirmPicker">确定</t-button></div>
    </t-dialog>
  </div>
</template>
...
const selectedAudios = ref<Array<{ id: string; name: string; duration: string; size: string; color: string; chapter_name: string }>>([]);
...
function confirmPicker() {
  if (pickerSelectedKeys.value.length === 0 && pickerTab.value === 'library') { MessagePlugin.warning('请至少选择一个音频'); return; }
  if (pickerTab.value === 'library') {
    pickerSelectedRows.value.forEach(r => {
      if (!selectedAudios.value.find(v => v.id === r.id)) {
        selectedAudios.value.push({ id: r.id, name: r.name, duration: r.duration, size: r.size, color: r.color, chapter_name: r.name.replace('.mp3', '') });
      }
    });
  } else {
    const mockFiles = [
      { id: `local-${Date.now()}-1`, name: '本地音频-01.mp3', duration: '15:00', size: '22MB', color: '#F8BBD0' },
      { id: `local-${Date.now()}-2`, name: '本地音频-02.mp3', duration: '18:30', size: '28MB', color: '#C5E1A5' },
    ];
    mockFiles.forEach(f => selectedAudios.value.push({ ...f, chapter_name: f.name.replace('.mp3', '') }));
  }
  showPicker.value = false;
  pickerSelectedKeys.value = [];
  pickerSelectedRows.value = [];
  MessagePlugin.success(`已选择 ${selectedAudios.value.length} 个音频`);
}

function onLocalUpload() { MessagePlugin.info('原型演示：模拟本地上传'); }
function removeSelected(item: any) { selectedAudios.value = selectedAudios.value.filter(v => v.id !== item.id); }

const batchForm = ref({
  detail_mode: 'full' as 'full' | 'brief', description: '',
  sale_type: 'free' as 'free' | 'paid' | 'member', price: '', original_price: '',
  member_scope: 'all' as 'all' | 'specific', member_levels: [] as string[],
  paid_validity: 'long' as 'long' | 'custom' | 'fixed', paid_custom_date: null as Date | null, paid_fixed_days: 365,
...
function doSave() {
  ...
  setTimeout(() => { submitting.value = false; MessagePlugin.success(`已批量创建 ${selectedAudios.value.length} 个音频`); router.push('/tenant/course/audios'); }, 600);
}
...
.item-thumb { width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; color: #1F2C3E; flex-shrink: 0; }
...
.chapter-thumb { width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 18px; color: #1F2C3E; flex-shrink: 0; }
