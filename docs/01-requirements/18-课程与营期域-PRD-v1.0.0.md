# 18-课程与营期域-PRD-v1.0.0

> **业务域**：课程与营期域（D18） | **版本**：v1.0.0 | **日期**：2026-08-18
> **需求流**：SaaS-Class | **类型**：新增 | **优先级**：P0
> **数据来源**：脑暴确认稿 v1.0.0（D1-D35）+ SugarMate 课程模块逆向分析报告 v1.0.0 + SugarMate 源码 + SugarMate PRD v2.3.0
> **追溯链**：UC-COURSE → FN-COURSE → PG-COURSE → SC-COURSE → BG-COURSE → BO-COURSE
> **系统缩写**：SAAS | **终端缩码**：PC（运营后台/租户后台）+ APP（学员端H5 375px）
> **V1核心策略**：基于 SugarMate 1:1 逆向分析重建，代码结构 1:1 对齐（zustand→pinia、antd→el-、tsx→vue）
> **PRD状态**：Draft（待评审）
...
| 角色 | 说明 | 权限边界 |
|------|------|---------|
| **讲师**（Lecturer） | 分主讲讲师 + 助教；role_type 通用（expert/assistant） | 主讲：讲课/出题/审核/答疑Admin；助教：拉新/答疑Member/助播/出题（主讲审核） |
| **学员**（Student） | 报名营期/独立学习；收红包/积分 | 报名/支付/学习/答题/打卡/评价/收红包/提现 |
| **管理员**（Admin） | PC 后台运营 | 课程审核/营期管理/报名审核/订单/分成结算/提现审批/退款/证书/评价审核 |

### 10.2 讲师角色类型（D1）

| 字段 | 说明 |
|------|------|
| `role_type` | 通用标签（如「营养专家」「健身教练」），展示用，不影响权限 |
| `can_be_main` | 是否可主讲（配置字段，非硬编码） |
| `can_be_assistant` | 是否可助教（配置字段） |

### 10.3 答疑权限矩阵（D19，1:1 对齐 SugarMate D5）
...
| BR-REFUND-001 | 退款原因 6 选 1（v1.1.0 补）：退款申请须选择预设原因（不想要了/质量不符/内容与描述不符/讲师服务差/个人时间冲突/其他），必选不可空 |
...
| BR-CERT-001 | 证书发放条件（D8）：课程完成率100% + 打卡≥80% + 测验通过 |
...
| course_id | string | 父课程ID（R-10 父子双向） |
...
| status | 'not_started'\|'live'\|'ended'\|'replay' | 直播状态 |
...
|------|------|:---:|:---:|
| FN-COURSE-PC-001 | 讲师库管理（CRUD+资质审核+导入成员+红包规则配置） | P0 | LecturerManagePage + 红包规则 |
...
| FN-COURSE-PC-014 | 课程评价审核（讲师/助教回复） | P0 | CourseReviewManagePage |
| FN-COURSE-PC-015 | 证书管理（查看/撤销/补发） | P1 | CertificateManagePage |
| FN-COURSE-PC-016 | 营期数据看板 | P1 | CampDashboardPage |
| FN-COURSE-PC-017 | 红包规则管理（新增） | P1 | RedPacketRuleManagePage |
| FN-COURSE-PC-018 | 钱包流水查看（新增） | P1 | WalletTransactionPage |
| FN-COURSE-PC-019 | 学员提现审核（新增） | P1 | StudentWithdrawReviewPage |
| FN-PC-002A | 课程类型管理（v1.1.0 新增） | P1 | CourseTypeManagePage |
| FN-PC-LIVE-001 | 直播场次管理（v1.1.0 新增） | P1 | LiveSessionManagePage |
| FN-PC-LIVE-002 | 直播中控台（v1.1.0 新增） | P1 | LiveControlCenterPage |

#### FN-COURSE-PC-001 讲师库管理
**用户故事**：作为管理员，我需要管理讲师库（CRUD+资质审核+导入成员+配置红包规则），以便维护讲师资源池。

**功能点**：
1. 讲师列表（表格：编号/姓名/角色类型/手机号/来源/审核状态/在职状态/累计课程/累计分成/操作）
2. 新增讲师（表单：name/phone/role_type/can_be_main/can_be_assistant/cert_no/institution/title/bio）
3. 从成员管理导入（选成员→mapMerchantRoleToLecturer 自动映射）
4. 资质审核（pending→approved/rejected，驳回填备注）
5. 讲师状态流转（active→suspended/left，离职后课程/营期讲师快照不失效 D16）
6. 建立讲师-助教归属关系（1讲师→N助教）
7. 红包规则配置（createRedPacketRule：ruleType/amount/dailyLimit）

**调用 Action**：createLecturer/importLecturerFromMerchant/updateLecturer/approveLecturer/transitionLecturerStatus/createAssistantRelation/createRedPacketRule

**验收标准**：
- ✅ 讲师列表可按姓名/手机号/角色/状态筛选
- ✅ 资质审核 pending→approved 后讲师可被课程引用
- ✅ 讲师离职（left）后已发布课程 lecturer_name 快照不失效
- ✅ 红包规则创建后 status=active，可配置 dailyLimit 防刷
...
#### FN-COURSE-PC-017 红包规则管理（新增）
**用户故事**：作为管理员/讲师，我需要管理红包规则。

**功能点**：
1. 红包规则列表（表格：规则ID/所有者/类型/金额/每日上限/状态/操作）
2. 新增规则（ruleType=new_member/completion/answer_correct，amount，dailyLimit）
3. 暂停/启用规则（status=active/paused）
4. 规则使用统计（已发放次数/总金额）

**调用 Action**：createRedPacketRule/loadRedPacketRules/updateRedPacketRule

**验收标准**：
- ✅ dailyLimit 防刷（BR-RED-008）
- ✅ 规则状态 active/paused/exhausted

#### FN-COURSE-PC-018 钱包流水查看（新增）
**用户故事**：作为管理员，我需要查看钱包流水。

**功能点**：钱包流水列表（表格：流水号/钱包/类型/金额/关联/状态/时间）+ 筛选（类型+状态+时间）。

**调用 Action**：loadWalletTransactions
...
| FN-COURSE-APP-012 | 讲师工作台（分成+提现+红包规则+钱包） | P1 | LecturerWorkbenchPage |
| FN-COURSE-APP-013 | 学员钱包+红包记录+提现（新增） | P1 | StudentWalletPage |
| FN-COURSE-APP-014 | 讲师充值入口（新增） | P1 | LecturerRechargePage |
...
### 14.12 LiveSession 状态机（4状态，v1.1.0 新增）
| 当前状态 | 可流转到 | 触发 action | 守卫条件 |
|---------|---------|------------|---------|
| not_started | live | startLiveSession | 关联 LiveRoom 状态=active + 讲师未离职 |
| live | ended | endLiveSession | 直播开始后才可结束 |
| ended | replay | 回放处理完成（异步回调） | 存在 replay_url |
| replay | —（终态） | — | 课程可消费 source=live_replay（D3） |

> 状态名对齐 ENT-LIVE-001 LiveSession.status（not_started/live/ended/replay），与 §12.7 实体定义一致。
...
| D9 | 优化 | 金额统一为分 |
| D10 | 优化 | 分成比例营期配置 |
| D11 | 优化 | 分成线下打款+讲师钱包红包用 |
| D12 | 优化 | APP报名真实落store |
| D13 | 优化 | 退款4项回滚完整 |
| D14 | 优化 | 完播率90%统一 |
| D15 | 优化 | 营期状态机流转明确 |
| D16 | 优化 | 讲师快照锁定 |
...
| D21 | 边界 | 做的范围 |
| D22 | 边界 | 不做范围 |
| D22-A | 边界 | 直播场次管理 v1.1.0 纳入范围（局部推翻 D22）：仅做场次管理+挂车+中控台，不做推流/观看/弹幕底层能力 |
| D23 | 红包 | 红包体系引入 |
| D24 | 积分 | 积分获取渠道 |
| D25 | 边界 | 提现门槛 |
| D26 | 边界 | 营期模式不可改 |
...
| D28 | 边界 | 证书撤销补发 |
| D29 | 红包 | 资金来源方案B |
| D30 | 红包 | RedPacketRule实体 |
| D31 | 红包 | RedPacketRecord幂等键含campId |
| D32 | 红包 | Wallet自动创建学员钱包 |
| D33 | 红包 | WalletTransaction移除预支 |
| D34 | 红包 | 红包发放闭环资金守恒 |
| D35 | 红包 | 红包配置可共存 |
