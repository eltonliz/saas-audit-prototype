...
| **店长/店员** | SaaS 门店成员 | 客户归属承接（营期报名客户自动归属，业绩归门店）；课程业务不提供归属调整入口 |
...
### 10B. V2·0901 用户裁决（本轮增量，优先级最高）

| 编号 | 裁决 | 说明 | 关联 |
|------|------|------|------|
| V2·0901-1 | 评价模块下线 | APP 评价 Tab/写评价页、PC 评价审核页、评价实体与数据全部移除；课程评分/评价数不再聚合 | BR-COURSE-009/015、§13 |
| V2·0901-2 | 授课方式不固定 | 课程/营期不再锁定直播/录播；课时级 mode 为准，组课与排课支持直播+录播混合；创建表单「授课方式」降级为默认值 | BR-COURSE-016、§13 |
| V2·0901-3 | 素材管理入口下线 | 视频课程/音频课程菜单与路由下线（内容池数据保留供组课引用）；「查看音频」列删除 | §13 |
| V2·0901-4 | 排课以课时为单位 | 新增/批量排课必选具体课时（跨课程平铺选择）；本营期已排课时禁选；直播排课只选直播间不关联课程 | §13 FN-PC-009 |
| V2·0901-5 | 排课明细可见性 | 门店开关「学员端隐藏每日课程明细」默认开启；APP 营期学习 Day 卡只显示日期与状态；当天直播在首页「今日直播」展示 | BR-COURSE-016 补充 |
| V2·0901-6 | 签到前置红包 | 领取课堂红包需当日已完成商城签到（每日+2积分、当日幂等）；APP 积分中心提供签到入口 | BR-RED-026 |
| V2·0901-7 | 打卡功能下线 | 营期打卡（看播打卡/课时打卡）随 BR-LEARN-004 废止同步下线；红包前置改为商城签到 | BR-LEARN-004 |
| V2·0901-8 | 复刻菜单精简 | 场次管理/红包记录/客户列表（复刻）入口下线；「课程报名」事件与来源全量移除（报名不再送积分） | §13 FN-GLUE |
...
| BR-COURSE-014 | **画面方向自动判定**（V2·0831）：课时视频上传时读取视频宽高，宽<高自动标记 portrait（竖屏）、否则 landscape，**不可手动修改**；课程 orientation 取首个课时视频方向（冗余存储）；APP 渲染：看课页播放器横屏 16:9 / 竖屏 9:16，课程详情头图横屏 2:1 / 竖屏 3:4 +「竖屏课程」角标；同一课程课时方向应一致，混选时以首个视频方向为准并表单提示 |
| BR-COURSE-015 | ~~评价模块~~（V2·0901 评价模块整体下线，本条废止；评价相关规则/页面/数据全部移除） |
| BR-COURSE-016 | **授课方式不固定**（V2·0901）：课程不再锁定直播/录播模式；课时级 mode 为准，支持直播+录播混合组课与混合排课；课程库内容管理支持视频/音频混选；课程表单「授课方式」降级为默认值 |
| BR-COURSE-017 | **素材引用锁定**：被课程引用的素材不可删除；批量添加格式「类型,标题,时长秒」逐行解析 |
...
| BR-ENROLL-012 | **报名落客户**：按手机号幂等创建/关联 SaaS 复刻客户（source=营期报名），learn_camp_count+1；V2·0901 报名积分下线（不再落报名积分流水） |
...
| BR-RED-023 | **积分任务**：课时完课(20)/课时答题(10)（SaaS 积分任务设置同步配置）；~~课程报名(10)~~ V2·0901 报名积分下线 |
| BR-RED-024 | **发放自动化**：全部激励由事件触发自动发放，无人工干预；发放记录回流营销域红包记录与客户积分流水 |
| BR-RED-025 | **客户360 回流**：红包记录按手机号关联客户；积分流水按客户关联；行为轨迹按时间倒序合并展示 |
| BR-RED-026 | **签到前置**（V2·0901）：领取课堂红包（答题/完课）需**当日已完成商城签到**（复用 SaaS「每日签到」积分任务，每日+2积分、当日幂等）；未签到时弹引导，一键签到后继续领取；APP 积分中心提供「每日签到」入口 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id / customer_id / event / reason | - | 事件（管理员发放/完课奖励/答题奖励·V2·0901 报名积分下线） |
...
| source | 'APP注册'\|'门店导入'\|'营期报名' | 来源（课程域新增枚举·V2·0901 改名） |
...
字段：id/customer_id/event（管理员发放|完课奖励|答题奖励·V2·0901 报名积分下线）/reason/type/operate_at/expire_at/status/delta/related_order_no。
...
| ~~FN-PC-003~~ | ~~视频课程~~（V2·0901 管理入口下线，内容池数据保留供组课引用） | — | VideoListPage（死文件） |
| ~~FN-PC-004~~ | ~~音频课程~~（V2·0901 管理入口下线） | — | AudioListPage（死文件） |
| FN-PC-005 | 题目库 | P0 | QuestionBankManagePage |
| ~~FN-PC-006~~ | ~~课程评价审核~~（V2·0901 评价模块整体下线） | — | CourseReviewManagePage（死文件） |
...
1. **客户列表**：1:1 线上列（22 列）+ 批量操作；红框新增=学习数据 4 列（学习课程数/参与营期数/学习时长/平均完课率）+ 客户来源「营期报名」+ 积分事件/红包场景课程枚举；积分管理弹窗（明细含课程事件）；客户详情 360（基础信息/数据概览含学习 4 项红框/行为轨迹红框：积分课程行为+红包领取合并时间线）。
2. **观看奖励**：1:1 规则列表；红框=红包类型新增「完课红包/答题红包」、绑定场景新增「营期」。
3. **红包记录**：1:1 记录列表；红框=红包场景新增「课程完课/课时答题」。
4. **积分任务设置**：1:1 任务列表；红框=新增课时完课(20)/课时答题(10)（V2·0901 课程报名任务下线）。
...
| **参与者** | 系统 |
| **前置条件** | 学员报名成功 |
| **主流程** | 1.createEnrollment → 2.按手机号查复刻客户 → 3.已存在：learn_camp_count+1 → 4.不存在：建档（来源=营期报名，学习数据初始 0） |
...
- **Then** 复刻客户新建（来源=营期报名）+ learn_camp_count=1 + 再次报名营期 B 时**不新建客户**，learn_camp_count+1（不再重复落积分）
...
营期 published → openEnrollment(enrolling)
  ...
  → createEnrollment(直接 enrolled，V2-03 免审核)
    → 联动：customer-replica-store.upsertCustomerFromEnrollment
      （按手机号幂等建档 source=营期报名；learn_camp_count+1；V2·0901 报名积分下线）
  → 提示「已报名，等待开营」
