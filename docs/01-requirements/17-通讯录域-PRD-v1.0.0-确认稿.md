...
**备选流程**：4a. 拒绝→已拒绝（24h 限再申请 3 次）；5a. 删除好友→仅清本地记录，**云端漫游保留**（Q-A 合规）；**拉黑后单聊互发拦截并提示「对方已被你拉黑，消息未发送」**（本期已实现）；2a. **等待验证期间禁止重复申请**（搜索结果显示「已申请」置灰，BR-IM-008b）；2b. **禁用/待审核/拉黑状态不可被添加**（搜索结果按钮置灰，BR-IM-008b）；3a. **申请记录保留 7 天**，超期自动清理（BR-IM-008b）
...
| BR-IM-008b | 好友申请补充规则：①等待验证（waiting）期间禁止重复发起，搜索结果显示「已申请」置灰；②禁用/待审核/拉黑状态不可被添加好友（按钮置灰，禁用→「已禁用」、待审核→「待审核」、拉黑→「已拉黑」）；③申请记录保留 7 天，超期自动清理（新的朋友页不再展示） |
